...
struct rpg_s *main)
{
    ...
    click_button_resume2(m, window, main, mouse);
    return m;
}

main_menu *click_button_settings(main_menu *m, sfRenderWindow *window,
struct rpg_s *main)
{
    sfVector2i mouse = sfMouse_getPositionRenderWindow(window);
    ...
    mouse.y > m->button_settings_pos.y &&
        ...
        m->rect_settings_button.top = 153;
...
main_menu *click_button_quit(main_menu *m, sfRenderWindow *window)
{
    sfVector2i s = sfMouse_getPositionRenderWindow(window);
    if ((m->version == 0 || m->version == 4) &&
        s.x > m->button_quit_pos.x && s.x < (m->button_quit_pos.x + 160) &&
        s.y > m->button_quit_pos.y && s.y < (m->button_quit_pos.y + 160)) {
        m->rect_quit_button.top = 153;
        sfSprite_setTextureRect(m->sprite_button_quit,
        m->rect_quit_button);
        sfSprite_setPosition(m->sprite_button_quit,
        m->button_quit_pos);
        update_version_quit(m);
    } else {
        ...
        sfSprite_setPosition(m->sprite_button_quit,
        m->button_quit_pos);
        m->is_quit_click = sfFalse;
        m->is_quit_click_ingame == sfFalse;
    }
    return m;
}
...
struct rpg_s *main)
{
    click_button_start(menu_pt, window, main);
    click_button_resume(menu_pt, window, main);
    click_button_resume_ingame(menu_pt, window, main);
    click_button_settings(menu_pt, window, main);
    click_button_quit(menu_pt, window);
    click_sett_no(main, window);
    click_sett_sound(main, window);
    click_sett_sound2(main, window);
    click_sett_window(main, window);
    click_button_help(menu_pt, window, main);
    click_button_return(menu_pt, window, main);
    click_button_quit_game_over(main, window);
    click_button_inventory(menu_pt, window, main);
    click_button_sett_help(menu_pt, window, main);
    click_button_save(menu_pt, window, main);
}
